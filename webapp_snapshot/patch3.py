  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [salesRes, comRes, catRes] = await Promise.all([
          fetch(`/api/sales?period=${periodStr}&strictPeriod=1`),
        ...
        setSales(salesData.logs || [])
        setCatalogs(catData || {})

        const parsedPyme = renderDashboardData(comData.pyme?.rules || {}, 'pyme')
        const parsedCaptador = renderDashboardData(comData.captador?.rules || {}, 'captador')
...
with open('src/app/liquidacion/rentabilidad-tiendas/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Complete overwrite successful.")
