...
export default function TerritorialPage() {
  ...
  const countSalesForStoreAndType = (storeName: string, tipoVenta: string) => {
    ...
    // El "tipoVenta" de la regla suele machear con el "producto" o con el "detalle"
              ...
              {tiendasRules.map((rule, idx) => {
                ...
                return (
                  <tr key={rule.id} style={{ borderBottom: '1px solid var(--border-color)', background: idx % 2 === 0 ? 'var(--bg-card)' : 'var(--section-bg)' }}>
                    ...
                    <td style={{ padding: 8 }}>
                      <select value={rule.tipoVenta} onChange={e => { const r = [...tiendasRules]; r[idx].tipoVenta = e.target.value; setTiendasRules(r); }} className="form-input" style={{ width: '100%' }}>
              ...
              {o2Rules.map((rule, idx) => {
                ...
                return (
                  <tr key={rule.id} style={{ borderBottom: '1px solid var(--border-color)', background: idx % 2 === 0 ? 'var(--bg-card)' : 'var(--section-bg)' }}>
                    ...
                    <td style={{ padding: 8 }}>
                      <select value={rule.tipoVenta} onChange={e => { const r = [...o2Rules]; r[idx].tipoVenta = e.target.value; setO2Rules(r); }} className="form-input" style={{ width: '100%' }}>
                        <option value="">Seleccionar</option>
                        {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </td>
