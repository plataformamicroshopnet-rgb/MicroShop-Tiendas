...
# 1. Rewrite SectionTable to always render un-grouped and without Pago por Tramo
...
if start_idx != -1:
  ...
  return (
    ...
    content = content[:start_idx] + new_section_table + content[end_idx+5:]

# 2. Fix the SectionTable calls correctly without breaking TS syntax
# We use regex to replace `<SectionTable label="..." ... />` with just what we need
content = re.sub(
    r'<SectionTable\s+label="([^"]+)"\s+badge="([^"]+)"\s+badgeColor="([^"]+)"\s+groups=\{([^}]+)\}\s+tabColor=\{([^}]+)\}[^>]+/>',
    r'<SectionTable label="\1" badge="\2" badgeColor="\3" groups={\4} tabColor={\5} />',
    content
)

# 3. Clean up the Excel export: remove 'Pago por Tramo' columns
# Excel sheet headers:
content = content.replace("'Pago por Tramo': fmtN(tramo)", "")
content = content.replace(",\n            })", "\n            })")
content = content.replace("'Pago por Tramo': fmtN(ea.telecomRewardAmount || 0),", "")
content = content.replace("'Pago por Tramo': fmtN(ea.telecomRewardAmount || 0)", "")
content = content.replace("'Pago por Tramo (\\')': fmtN(plusTramo + basicoTramo),", "")
content = content.replace("'Pago por Tramo (\\')': fmtN(extrasTotal)", "")
content = content.replace("'Pago por Tramo (\\')': fmtN(summaryRows.reduce((a, r) => a + (r['Pago por Tramo (\\')'] as number), 0)),", "")
content = content.replace(",  }", " }")

# 4. Remove UI mentions of Pago por Tramo
content = re.sub(r'\{ label: \'PAGO POR TRAMO\',\s+right: true\s+\},', '', content)
content = re.sub(r'El <strong>Pago por tramo</strong> se calcula\s+autom\w+ticamente usando las tablas de comisiones \(Ventas vs Importes PLUS / B\w+SICO\) y el % cumplimiento\s+acumulado del per\w+odo activo.', '', content)
