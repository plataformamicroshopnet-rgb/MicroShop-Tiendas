...
filepath = r'src\app\api\movilfree\sales\[id]\route.ts'
with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# I will replace the entire PUT function to safely add partial returns
new_put = """export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
...
content = re.sub(
    r"export async function PUT\(req: Request.*?(?=export async function DELETE)",
    new_put + "\n\n",
    content,
    flags=re.DOTALL
)
...
print("Updated backend sales API for partial returns")
