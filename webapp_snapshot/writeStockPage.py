...
export default function StockPage() {
  ...
  const parseNumber = (val: string) => {
    if (!val) return 0
    const clean = val.replace(/€/g, '').replace(/\\s/g, '').replace(',', '.')
  ...
  const handleBulkImport = async () => {
    ...
    for (const line of lines) {
      ...
      if (activeTab === 'DISPOSITIVOS OJO') {
        ...
        item.udsMovilfree = parseIntSafe(parts[7])
  ...
  const getTotalsByStoreAndCategory = () => {
    const cats = ['Accesorios', 'Rent', 'Demos', 'DISPOSITIVOS OJO']
    let matrix: Record<string, any> = {}
    cats.forEach(c => {
      matrix[c] = { Uds: 0, Valor: 0, Correhuela: 0, Auxiliadora: 0, Bejar: 0, Villamayor: 0 }
    })
        ...
        {loading ? (
          ...
          <div>
             ...
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
                ...
                {/* Cuadro de Accesorios por Tienda */}
                ...
                <div style={{ border: '1px solid #E2E8F0', borderRadius: '12px', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '16px' }}>
                       ...
                       <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #F1F5F9', background: '#DBEAFE', borderRadius: 4, paddingLeft: 8, paddingRight: 8, marginBottom: 4 }}>
                           ...
                           <span style={{ fontWeight: 'bold', color: '#1E3A8A' }}>{totalsMatrix['DISPOSITIVOS OJO'].Uds} uds / {formatEuro(totalsMatrix['DISPOSITIVOS OJO'].Valor)}</span>
                       </div>
                       ...
                       <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', background: '#0EA5E9', borderRadius: 4, paddingLeft: 8, paddingRight: 8, color: '#FFF', marginTop: 12 }}>
                           ...
                           <span style={{ fontWeight: 'bold', fontSize: 18 }}>
                               {totalsMatrix['Accesorios'].Uds + totalsMatrix['DISPOSITIVOS OJO'].Uds + totalsMatrix['Rent'].Uds + totalsMatrix['Demos'].Uds} uds / 
                               {formatEuro(totalsMatrix['Accesorios'].Valor + totalsMatrix['DISPOSITIVOS OJO'].Valor + totalsMatrix['Rent'].Valor + totalsMatrix['Demos'].Valor)}
                           </span>
                       </div>
                    </div>
                </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' }}>
                <thead>
                  <tr style={{ background: '#F1F5F9', borderBottom: '2px solid #E2E8F0' }}>
                    <th style={{ padding: '12px 16px', color: '#475569', fontWeight: 600, fontSize: '13px', width: '25%' }}>{activeTab === 'Accesorios' ? 'Accesorio' : 'Marca y Modelo'}</th>
                    ...
                    <th style={{ padding: '12px 16px', color: '#475569', fontWeight: 600, fontSize: '13px', textAlign: 'center', width: '8%' }}>Movilfree</th>
                ...
                <tbody>
                  {filteredItems.length === 0 ? (
                    <tr>
                      <td colSpan={10} style={{ padding: '32px 16px', textAlign: 'center', color: '#64748B' }}>
