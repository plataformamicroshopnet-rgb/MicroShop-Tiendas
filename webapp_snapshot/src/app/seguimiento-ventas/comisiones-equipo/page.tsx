    ...
    return (
        <div style={{ padding: '24px 32px', backgroundColor: 'var(--bg-app)', minHeight: '100vh' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px', margin: '20px 0' }}>
                <div className="card" style={{ padding: '10px 16px', background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.1) 0%, rgba(139, 92, 246, 0.05) 100%)', border: '1px solid rgba(168, 85, 247, 0.3)', borderLeft: '4px solid #a855f7' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <div style={{ background: 'rgba(168, 85, 247, 0.2)', padding: 6, borderRadius: 8 }}><DollarSign color="#a855f7" size={16} /></div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Inversión Total Comisiones</div>
                    </div>
                    <div style={{ fontSize: 24, fontWeight: 900, color: 'var(--text-main)' }}>
                        {teamTotalComisiones.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span style={{ color: '#a855f7', fontSize: 14 }}>€</span>
                    </div>
                </div>

                <div className="card" style={{ padding: '10px 16px', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(52, 211, 153, 0.05) 100%)', border: '1px solid rgba(16, 185, 129, 0.3)', borderLeft: '4px solid #10b981' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <div style={{ background: 'rgba(16, 185, 129, 0.2)', padding: 6, borderRadius: 8 }}><TrendingUp color="#10b981" size={16} /></div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Volumen de Ventas Equipo</div>
                    </div>
                    <div style={{ fontSize: 24, fontWeight: 900, color: 'var(--text-main)' }}>
                        {teamTotalSales.toLocaleString('es-ES')} <span style={{ color: '#10b981', fontSize: 14 }}>operaciones</span>
                    </div>
                </div>
            </div>
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden', border: '1px solid var(--border-strong)', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
                <div style={{ padding: '10px 16px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--border-strong)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                    <h2 style={{ fontSize: 16, margin: 0, display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-main)' }}>
                        <Users size={16} color="#3b82f6" /> Ranking Comercial
                    </h2>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', padding: 2, borderRadius: 8 }}>
                            <button onClick={() => setViewMode('comerciales')} style={{ background: viewMode === 'comerciales' ? '#3b82f6' : 'transparent', color: viewMode === 'comerciales' ? '#fff' : 'var(--text-muted)', padding: '4px 10px', borderRadius: 6, cursor: 'pointer', fontWeight: 600, border: 'none', transition: 'all 0.2s', fontSize: 12 }}>Por Comercial</button>
                            <button onClick={() => setViewMode('comisiones')} style={{ background: viewMode === 'comisiones' ? '#a855f7' : 'transparent', color: viewMode === 'comisiones' ? '#fff' : 'var(--text-muted)', padding: '4px 10px', borderRadius: 6, cursor: 'pointer', fontWeight: 600, border: 'none', transition: 'all 0.2s', fontSize: 12 }}>Por Nombre Comisión</button>
                        </div>
                        {viewMode === 'comerciales' && (
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={() => setSortKey('totalComision')} style={{ background: sortKey === 'totalComision' ? 'rgba(168, 85, 247, 0.2)' : 'transparent', border: `1px solid ${sortKey === 'totalComision' ? '#a855f7' : 'var(--border-strong)'}`, color: sortKey === 'totalComision' ? '#a855f7' : 'var(--text-muted)', padding: '4px 10px', borderRadius: 12, cursor: 'pointer', fontWeight: 600, transition: 'all 0.2s', fontSize: 12 }}>Por Comisión</button>
                                <button onClick={() => setSortKey('totalSales')} style={{ background: sortKey === 'totalSales' ? 'rgba(16, 185, 129, 0.2)' : 'transparent', border: `1px solid ${sortKey === 'totalSales' ? '#10b981' : 'var(--border-strong)'}`, color: sortKey === 'totalSales' ? '#10b981' : 'var(--text-muted)', padding: '4px 10px', borderRadius: 12, cursor: 'pointer', fontWeight: 600, transition: 'all 0.2s', fontSize: 12 }}>Por Ventas</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: viewMode === 'comerciales' ? 900 : 600 }}>
                        {viewMode === 'comerciales' ? (
                            <>
                                <thead>
                                    <tr style={{ background: 'rgba(0,0,0,0.2)', textTransform: 'uppercase', fontSize: 11, color: 'var(--text-muted)', letterSpacing: 0.5 }}>
                                        <th style={{ padding: '6px 12px', width: 60, textAlign: 'center' }}>Pos</th>
                                        <th style={{ padding: '6px 12px' }}>Comercial</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'center' }}>Perfil</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'center' }}>Ventas</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'center' }}>Pdte.</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'right' }}>Com. Base</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'right' }}>Extras</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'right' }}>Total Comisión</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {sortedStats.map((s, index) => {
                                        const rank = index + 1;
                                        let rankBadge = <div style={{ fontWeight: 800, color: 'var(--text-muted)', fontSize: 14 }}>{rank}</div>;
                                        if (rank === 1) rankBadge = <Award size={20} color="#fbbf24" fill="rgba(251, 191, 36, 0.2)" />;
                                        else if (rank === 2) rankBadge = <Award size={18} color="#94a3b8" fill="rgba(148, 163, 184, 0.2)" />;
                                        else if (rank === 3) rankBadge = <Award size={16} color="#b45309" fill="rgba(180, 83, 9, 0.2)" />;
                                            ...
                                            <tr key={s.name} style={{ borderBottom: '1px solid var(--border-light)', transition: 'background 0.2s', cursor: 'default' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                                ...
                                                <td style={{ padding: '6px 12px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <div style={{ width: 26, height: 26, borderRadius: '50%', background: s.isPlus ? 'linear-gradient(135deg, #3b82f6, #2563eb)' : 'linear-gradient(135deg, #f59e0b, #d97706)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 13, boxShadow: '0 4px 10px rgba(0,0,0,0.3)' }}>
                                                ...
                                                <td style={{ padding: '6px 12px', textAlign: 'center' }}>
                                                    <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 700, background: s.isPlus ? 'rgba(59, 130, 246, 0.15)' : 'rgba(245, 158, 11, 0.15)', color: s.isPlus ? '#60a5fa' : '#fbbf24', border: `1px solid ${s.isPlus ? 'rgba(59, 130, 246, 0.3)' : 'rgba(245, 158, 11, 0.3)'}` }}>
                                                        {s.profile}
                                                    </span>
                                                </td>
                                                <td style={{ padding: '6px 12px', textAlign: 'center' }}>
                                                    <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-main)' }}>{s.totalSales}</div>
                                                </td>
                                                <td style={{ padding: '6px 12px', textAlign: 'center' }}>
                                                    {s.pendientes > 0 ? (
                                                        <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, color: '#f59e0b', fontWeight: 700, background: 'rgba(245, 158, 11, 0.1)', padding: '2px 8px', borderRadius: 12, fontSize: 11 }}>
                                                            <ShieldAlert size={12} /> {s.pendientes}
                                                        </div>
                                                    ) : (
                            ...
                            <>
                                <thead>
                                    <tr style={{ background: 'rgba(0,0,0,0.2)', textTransform: 'uppercase', fontSize: 11, color: 'var(--text-muted)', letterSpacing: 0.5 }}>
                                        <th style={{ padding: '6px 12px', width: 60, textAlign: 'center' }}>Pos</th>
                                        <th style={{ padding: '6px 12px' }}>Nombre Comisión</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'center' }}>Volumen (Ventas)</th>
                                        <th style={{ padding: '6px 12px', textAlign: 'right' }}>Inversión (Comisiones)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        return (
                                            <tr key={r.name} style={{ borderBottom: '1px solid var(--border-light)', transition: 'background 0.2s', cursor: 'default' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                                                ...
                                                <td style={{ padding: '6px 12px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        {r.isExtra ? (
                                                            <div style={{ width: 26, height: 26, borderRadius: '6px', background: 'rgba(168, 85, 247, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a855f7' }}>
                                                                <Star size={14} />
                                                            </div>
                                                        ) : (
                                                            <div style={{ width: 26, height: 26, borderRadius: '6px', background: 'rgba(59, 130, 246, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3b82f6' }}>
                                                                <DollarSign size={14} />
                                                            </div>
                                                        )}
                                                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-main)' }}>{r.name === 'ARPU' ? 'Arpu (Repos)' : r.name}</div>
                                                    </div>
                                                </td>
                                                <td style={{ padding: '6px 12px', textAlign: 'center' }}>
