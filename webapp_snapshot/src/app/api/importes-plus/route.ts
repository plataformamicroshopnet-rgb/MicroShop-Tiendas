...
export async function GET(request: Request) {
  try {
    ...
    if (legacyOnly) {
      importes = await prisma.importePlus.findMany({
        where: { periodId: null },
    ...
    console.error('GET /api/importes-plus error:', error);
...
export async function POST(request: Request) {
  try {
    ...
    const nuevoImporte = await prisma.importePlus.create({
      data: {
    ...
    console.error('POST /api/importes-plus error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (body.operations && Array.isArray(body.operations)) {
       const deleteOperation = body.deletions && body.deletions.length > 0 
           ? [prisma.importePlus.deleteMany({ where: { id: { in: body.deletions } } })] 
           : []
       ...
       const upsertOperations = body.operations.map((item: any) => {
           if (item.id) {
               ...
               return prisma.importePlus.update({
                   where: { id: item.id },
                   data: updateData
               })
           } else {
               return prisma.importePlus.create({
                   data: {
    ...
    const importeActualizado = await prisma.importePlus.update({
      where: { id },
      data: updateData,
    });
    ...
    console.error('PUT /api/importes-plus error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    await prisma.importePlus.delete({
      where: { id },
    });

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('DELETE /api/importes-plus error:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
