...
export async function GET() {
  try {
    const items = await prisma.movilFreeStockTransfer.findMany({
      include: {
        product: true
      },
      orderBy: {
    ...
    const msItems = await prisma.microShopStockTransfer.findMany({
      include: {
        product: true
      },
      orderBy: {
    ...
    const combined = [...items, ...msItems].sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime())
...
export async function POST(req: Request) {
  try {
    ...
    const isMicroShop = await prisma.microShopProduct.findUnique({ where: { id: productId } })
    if (isMicroShop) {
