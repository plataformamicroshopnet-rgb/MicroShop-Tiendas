...
export default function ImportesPlusPage() {
  const [data, setData] = useState<ImportePlus[]>([])
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/importes-plus')
  ...
  const handleDeleteRow = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/importes-plus?id=${id}`, {
        method: 'DELETE'
      })
      const json = await res.json()
      if (json.success) {
  ...
  const handleChange = (id: string, field: keyof ImportePlus, value: string) => {
  ...
  return (
    <div style={{ padding: 20 }}>
      <PageHeader 
        title={<><Euro className="text-cyan" size={28} /> Importes Plus</>}
          ...
          <table style={{ minWidth: 1200, width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              ...
              <tr style={{ backgroundColor: '#B5C400', color: 'var(--bg-card)' }}>
                <th style={{ padding: '12px', textAlign: 'left', minWidth: 200, borderRight: '1px solid rgba(255,255,255,0.2)' }} rowSpan={2}>BLOQUES DE COBRO PLUS</th>
