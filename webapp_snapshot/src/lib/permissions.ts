...
export function getDefaultPermissions(role: string): string[] {
  switch (role) {
    case 'ADMIN':
      return [
        'MODULE_TIENDAS', 'MODULE_JEFE_TIENDAS', 'MODULE_BACK_OFFICE', 'MODULE_CUMPLIMIENTO',
    ...
    case 'JEFE DE VENTAS':
      return [
        'MODULE_TIENDAS', 'MODULE_JEFE_TIENDAS', 'MODULE_CUMPLIMIENTO', 'MODULE_COMISIONES',
        'MODULE_LIQUIDACION', 'MODULE_DIRECCION',
        'PRINT', 'EXPORT_EXCEL', 'SEND_EMAIL'
      ]
    case 'BACK OFFICE':
      return [
        'MODULE_TIENDAS', 'MODULE_BACK_OFFICE', 'MODULE_CUMPLIMIENTO',
        'CREATE_SALES', 'EDIT_SALES', 'CANCEL_SALES', 'PRINT', 'EXPORT_EXCEL', 'SEND_EMAIL'
      ]
    case 'COMERCIAL':
    default:
      return [
        'MODULE_TIENDAS', 'MODULE_CUMPLIMIENTO',
        'PRINT'
      ]
  }
}
...
export function canView(user: any, moduleName: string): boolean {
  ...
  // Legacy fallback: si tiene en bruto MODULE_TIENDAS, puede verlo
...
export function canEdit(user: any, moduleName: string): boolean {
  ...
  if (moduleName === 'MODULE_TIENDAS') return activePerms.includes('EDIT_SALES') || activePerms.includes('CREATE_SALES');
  ...
  if (moduleName === 'MODULE_JEFE_TIENDAS') return activePerms.includes('MODULE_JEFE_TIENDAS');
